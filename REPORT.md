...
` mv stage/lab ~/boost-libs`

10. Подсчитаем количество дискового пространства для каждого файла в текущем каталоге

`ls -shS` - где -s -  печатать размер каждого файла в блоках, h - печатать размеры в удобном для человека виде
                             (например, 1K 234M 2G), S -  сортировать по размеру файла (по убиванию)
                             
                             
 `ls -shS | head 10` - head - выводит начальные 10 строк 
